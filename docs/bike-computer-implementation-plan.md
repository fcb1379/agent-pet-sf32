...
范围状态：X-TRACK 原工程使用 Micro SD 存放地图和轨迹；当前 SF32 工程已按黄山派基板 pinmux 启用 SPI1 TF 与 SDK SPI-MSD，在 `/sd` 独立挂载已有 FAT 文件系统，并为轨迹/地图提供 TF 优先、板载 Elm FAT 回退。计步所需 IMU 仍未确认。
...
门禁：TF 引脚与驱动的源码/构建证据已具备；真实卡兼容性、异常拔卡和断电测试，以及 IMU 型号确认与计步准确率，放在所有代码检查之后。
