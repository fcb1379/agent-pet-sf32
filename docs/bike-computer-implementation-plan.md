...
当前进度：主数据、定位详情、骑行总结三页和阶段 6 的离线地图页已完成；主数据页提供开始/暂停与保存结束，总结页提供保存/丢弃，KEY2 可完成开始/暂停/继续和保存结束。`bikedemo on/off/status` 可在无 GPS 模组时每秒生成固定容量模拟定位，UART2 缺失时工作线程仍可启动，界面明确显示 `DEMO`。
...
范围状态：X-TRACK 原工程使用 Micro SD 存放地图和轨迹；当前 SF32 工程已按黄山派基板 pinmux 启用 SPI1 TF 与 SDK SPI-MSD，在 `/sd` 独立挂载已有 FAT 文件系统，并为轨迹/地图提供 TF 优先、板载 Elm FAT 回退。黄山派官方资料确认板载 LSM6DS3TR-C，当前 QADSPI 构建下已通过 I2C2 PA39/PA40 接入 SDK LSM6DSL 兼容计步驱动。
...
当前进度：TF 路径回退与板载计步均已完成源码、主机回归和目标构建。计步累计纯逻辑覆盖复位、16 位回绕、异常跳变和 32 位饱和；生成配置仅启用 LSM6DSL step，未注册不需要的加速度/陀螺仪设备。

门禁：TF 与计步的源码/构建证据已具备；真实卡兼容性、异常拔卡和断电测试，以及 WHO_AM_I、计步准确率、功耗和 PA39/PA40 实际信号，放在所有代码检查之后。
